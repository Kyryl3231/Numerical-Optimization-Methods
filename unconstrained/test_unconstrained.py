...
from unconstrained import steepest_descent
import sys
sys.path.append("..")
...
for fun,sol,name in zip(tf.test1_fun_list, tf.test1_solutions, tf.test1_fun_names):
    ...
    for x_0 in tf.test1_starting_points:
        ...
        minimizer, iter_count, stopping_criterion, stopping_values = steepest_descent(fun, x_0, max_iter=10**5)
...
for plot_idx in range(plot_num):
    ...
    minimizer, it_num, stopping_criterion, stopping_values = steepest_descent(tf.least_squares, x_0, a, b, gradient_fun=gradient_ls, max_iter=10**6)
